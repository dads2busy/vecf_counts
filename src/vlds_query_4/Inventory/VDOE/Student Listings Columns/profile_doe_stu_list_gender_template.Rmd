...
column_valid_values <-  dictionary[agency_name == "Virginia Department of Education"][view_name == "Unique Students Listing"][column_name == "Gender"][,c("value", "description")]
```


<!--UNIQUENESS-->
```{r uniqueness, include=FALSE, eval=uniqueness_print}
values <-  cust_by_yr[!is_blank(gender)|gender %in% column_valid_values$value, gender]
...
missing <- cust_by_yr[is_blank(gender), .N]
...
invalid = cust_by_yr[is_blank(gender)|!gender %in% column_valid_values$value, .N]
...
# get record details
  invalid_details <-
    cust_by_yr[is_blank(gender)|!gender %in% column_valid_values$value,
               c("unique_id", "gender", "school" )] 
