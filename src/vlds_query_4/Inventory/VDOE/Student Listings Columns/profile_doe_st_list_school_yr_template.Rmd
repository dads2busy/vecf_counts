...
cust_by_yr <- data.table::fread("~/git/vecf_counts/data/original/q4/DOE/Unique Students Listing.csv", colClasses = "character")
...
table_name = "DOE Student Listings"
...
column_valid_values <- dictionary[agency_name == "Virginia Department of Education"][view_name == "Unique Students Listing"][column_name == "School Year"][,c("value", "description")]
```


<!--UNIQUENESS-->
```{r uniqueness, include=FALSE, eval=uniqueness_print}
values <-  cust_by_yr[!is_blank(school_year)|school_year %in% column_valid_values$value, school_year]
...
missing <- cust_by_yr[is_blank(school_year), .N]
...
invalid = cust_by_yr[is_blank(school_year)|!school_year %in% column_valid_values$value, .N]
...
# get record details
  invalid_details <-
    cust_by_yr[!is_blank(school_year)|!school_year %in% column_valid_values$value,
               c("unique_id", "school_year", "division_number_reporting_school_numbe" )] 
...
record_inconsistent_compare_columns_1 <- c("Entry Date")

# consistency check definition
record_inconsistent_compare_1_definition <- 
  "Check if there any records that have an entry date outside of the given school year."

# get inconsistent records
# cust_by_yr <- cust_by_yr[, interval := lubridate::interval(start = lubridate::ymd(paste0(school_year, "-07-01")), end = lubridate::ymd(paste0(school_year, "06-30")))]
# #interval16 <- interval(start = lubridate::ymd("2016-07-01"), end = lubridate::ymd("2017-06-30"))


cust_by_yr <- cust_by_yr[, interval := lubridate::interval(start = lubridate::ymd(paste0(school_year, "-07-01")), end = lubridate::ymd(paste0(school_year+1, "06-30")))]

cust_int <- cust_by_yr[!is.na(lubridate::ymd(lubridate::as_date(entry_date))), ][ , check := ifelse(ymd(as_date(entry_date)) %within% interval, 1,  0)]
...
# get record details
  record_inconsistent_1_details <- cust_int[int <0, ][, c("school_year", "assessment_date")]
...
# create a list of the other columns to which the column is being compared
record_inconsistent_compare_columns_2 <- c("Birth Year")
...
record_inconsistent_2 <- 
  cust_by_yr[birth_year >= school_year][,.N]
...
# get record details
  record_inconsistent_2_details <-
    cust_by_yr[birth_year >= school_year][ , c("school_year", "birth_year")]
