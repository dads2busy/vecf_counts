...
column_valid_values <-  dictionary[agency_name == "Virginia Department of Education"][view_name == "Unique Students Listing"][column_name == "Race Type"][,c("value", "description")]
```


<!--UNIQUENESS-->
```{r uniqueness, include=FALSE, eval=uniqueness_print}
values <-  cust_by_yr[!is_blank(race_type)|race_type %in% column_valid_values$value, race_type]
...
missing <- cust_by_yr[is_blank(race_type), .N]
...
invalid = cust_by_yr[is_blank(race_type)|!race_type %in% as.character(c(1:6)), .N]
...
# get record details
  invalid_details <-
    cust_by_yr[is_blank(race_type)|!race_type %in% as.character(c(1:6)),
               c("unique_id", "race_type", "division_number_reporting_school_numbe" )] 
