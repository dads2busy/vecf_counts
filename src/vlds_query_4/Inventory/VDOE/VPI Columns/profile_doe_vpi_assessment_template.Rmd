...
column_valid_values <- "Non-date values are excluded."
```


<!--UNIQUENESS-->
```{r uniqueness, include=FALSE, eval=uniqueness_print}
values <-  cust_by_yr[!is.na(lubridate::mdy(assessment_date)), lubridate::mdy(assessment_date)]
...
frequencies$values <- paste(stringr::str_sub(frequencies$values, 6,7), 
        stringr::str_sub(frequencies$values, 1, 4), sep = "/")
...
missing <- cust_by_yr[is_blank(assessment_date), .N]
...
invalid = cust_by_yr[!is_blank(assessment_date),][is.na(lubridate::mdy(assessment_date)), .N]
...
# get record details
  invalid_details <-
    cust_by_yr[!is_blank(assessment_date),][is.na(lubridate::mdy(assessment_date)),
               c("unique_id", "assessment_date", "school" )] 
...
record_inconsistent_compare_columns_1 <- c("Birth Year")

# consistency check definition
record_inconsistent_compare_1_definition <- 
  "Check if there any records that have an assessment date prior to their birth year."

# get inconsistent records
cust_int <- cust_by_yr[!is.na(lubridate::mdy(assessment_date)), ][, begin := as.numeric(birth_year)][ , end := as.numeric(stringr::str_sub(lubridate::mdy(assessment_date), 1,4 ))] [ , int:= end - begin]
...
# get record details
  record_inconsistent_1_details <- cust_int[int <0, ][, c("assessment_date", "int")]
...
# plot inconsistencies by School Year
  school_year <- (ggplot(
    ...
    ylab("Frequency") +
    scale_x_discrete("Assessment Date", drop = "FALSE") +
    theme(panel.background=element_blank()))
  }
