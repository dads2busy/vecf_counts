...
column_valid_values <- dictionary[agency_name == "Virginia Department of Education"][view_name == "VPI+"][column_name == "Semester"][,c("value", "description")]
```


<!--UNIQUENESS-->
```{r uniqueness, include=FALSE, eval=uniqueness_print}
values <-  cust_by_yr[!is_blank(semester)|semester %in% c("Fall", "Spring"), semester]
...
missing <- cust_by_yr[is_blank(semester), .N]
...
invalid = cust_by_yr[is_blank(semester)|!semester %in% c("Fall", "Spring"), .N]
...
# get record details
  invalid_details <-
    cust_by_yr[is_blank(semester)|!semester %in% c("Fall", "Spring"),
               c("unique_id", "semester", "school" )] 
