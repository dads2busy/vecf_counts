...
column_valid_values <-  dictionary[agency_name == "Virginia Department of Education"][view_name == "VPI+"][column_name == "School"][,c("value", "description")]
```


<!--UNIQUENESS-->
```{r uniqueness, include=FALSE, eval=uniqueness_print}
values <-  cust_by_yr[!is_blank(school)|school %in% column_valid_values$value, school]
...
missing <- cust_by_yr[is_blank(school), .N]
...
invalid = cust_by_yr[is_blank(school)|!school %in% column_valid_values$value, .N]
...
# get record details
  invalid_details <-
    cust_by_yr[is_blank(school)|!school %in% column_valid_values$value,
               c("unique_id", "school", "school" )] 
