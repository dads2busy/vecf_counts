...
column_name = "beginning school year"
table_name = "DOE VPI+"
...
column_valid_values <- tibble::tibble("Year" = c(2015:2016))
```


<!--UNIQUENESS-->
```{r uniqueness, include=FALSE, eval=uniqueness_print}
values <-  cust_by_yr[!is_blank(beginning_school_year)|beginning_school_year %in% as.character(c(2010:2019)), beginning_school_year]
...
missing <- cust_by_yr[is_blank(beginning_school_year), .N]
...
invalid = cust_by_yr[is_blank(beginning_school_year)|!beginning_school_year %in% as.character(c(2010:2019)), .N]
...
# get record details
  invalid_details <-
    cust_by_yr[!is_blank(beginning_school_year)|!beginning_school_year %in% as.character(c(2010:2019)),
               c("unique_id", "beginning_school_year", "school" )] 
