...
column_valid_values <- tibble::tibble("Grade" = c("PreK", "K"))
```


<!--UNIQUENESS-->
```{r uniqueness, include=FALSE, eval=uniqueness_print}
values <-  cust_by_yr[!is_blank(grade)|grade %in% c("PreK", "K"), grade]
...
missing <- cust_by_yr[is_blank(grade), .N]
...
invalid = cust_by_yr[is_blank(grade)|!grade %in% c("PreK", "K"), .N]
...
# get record details
  invalid_details <-
    cust_by_yr[is_blank(grade)|!grade %in% c("PreK", "K"),
               c("unique_id", "grade", "school" )] 
