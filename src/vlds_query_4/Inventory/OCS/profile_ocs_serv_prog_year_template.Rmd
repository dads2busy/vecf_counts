...
column_valid_values <- "Values from 0 to 366, representing length of a year."
```


<!--UNIQUENESS-->
```{r uniqueness, include=FALSE, eval=uniqueness_print}
values <-  cust_by_yr[!is_blank(service_duration), service_duration]
...
missing <- cust_by_yr[is_blank(service_duration), .N]
...
invalid = cust_by_yr[is_blank(service_duration)|!service_duration %in% as.character(c(0:366)) , .N] 
...
# get record details
  invalid_details <-
    cust_by_yr[is_blank(service_duration)|!service_duration %in% as.character(c(0:366)), c("unique_id", "service_duration", "service_placement_type" )] 
