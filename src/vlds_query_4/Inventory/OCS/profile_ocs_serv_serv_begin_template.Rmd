...
record_inconsistent_compare_columns_1 <- c("Service End Date")

# consistency check definition
record_inconsistent_compare_1_definition <- 
  "Check if there any records that have a a service end date prior to their service start date."

# get inconsistent records
cust_int <- cust_by_yr[, begin := lubridate::as_date(lubridate::mdy(cust_by_yr$service_begin_date))][ , end := lubridate::as_date(lubridate::mdy(cust_by_yr$service_end_date))][, int := (lubridate::int_length(lubridate::interval(start = begin, end = end)))/86400]


record_inconsistent_1 <- cust_int[int <0, .N]
...
# get record details
  record_inconsistent_1_details <- cust_int[int <0, ][, c("begin", "int")]
...
# plot inconsistencies by School Year
  school_year <- (ggplot(
    year_plot_df, aes(begin, Freq)) +
    geom_bar(stat="identity", fill ="honeydew3", width=.7) +
    ggtitle("Inconsistent Record Entry by Service Start Date") +
    ylab("Values") +
    scale_x_discrete("Start Date", drop = "FALSE") +
    theme(panel.background=element_blank()))
