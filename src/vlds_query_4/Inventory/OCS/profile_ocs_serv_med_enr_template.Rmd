...
column_name = "clinical medication"
...
column_valid_values <-  dictionary[agency_name == "Office of Childrens Services"][view_name == "OCS Services By Year"][column_name == "Medicaid Enrolled"][, c("value", "description")]
```


<!--UNIQUENESS-->
```{r uniqueness, include=FALSE, cache=FALSE, eval=uniqueness_print}
values <-  cust_by_yr[!is_blank(medicaid_enrolled), medicaid_enrolled]
...
missing <- cust_by_yr[is_blank(medicaid_enrolled), .N]
...
invalid = cust_by_yr[is_blank(medicaid_enrolled)|!medicaid_enrolled %in% c("1", "2") , .N] 
...
# get record details
  invalid_details <-
    cust_by_yr[is_blank(medicaid_enrolled)|!medicaid_enrolled %in% c("1", "2"), c("unique_id", "medicaid_enrolled", "service_placement_type" )] 
