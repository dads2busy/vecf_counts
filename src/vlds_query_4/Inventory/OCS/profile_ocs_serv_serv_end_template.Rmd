...
values <-  cust_by_yr[!is_blank(service_end_date), service_end_date]

unique_values <- 
  length(unique(values))

# frequency distribution table of active_status values
frequencies <- as.data.frame(table( paste(stringr::str_sub(cust_by_yr$service_end_date, 1,2), stringr::str_sub(cust_by_yr$service_end_date, 7, 10), sep = "/")))
...
missing <- cust_by_yr[is_blank(service_end_date), .N]
...
invalid = cust_by_yr[is_blank(service_end_date)|
                       !stringr::str_sub(service_end_date, -4, -1) %in% c("2015", "2016") | 
                       !as.numeric(stringr::str_sub(service_end_date, -7, -6)) %in% c(1:31)| 
                       !as.numeric(stringr::str_sub(service_end_date, 1, 2)) %in% c(1:12), 
                     .N] 
...
# get record details
  invalid_details <-
    cust_by_yr[is_blank(service_end_date)|!stringr::str_sub(service_end_date, -4, -1) %in% c("2015", "2016") | 
                       !as.numeric(stringr::str_sub(service_end_date, -7, -6)) %in% c(1:31)| 
                       !as.numeric(stringr::str_sub(service_end_date, 1, 2)) %in% c(1:12),
               c("unique_id", "service_end_date", "service_placement_type" )] 
...
# get record details
  record_inconsistent_1_details <- cust_int[int <0, ][, c("end", "int")]
...
# plot inconsistencies by School Year
  school_year <- (ggplot(
    ...
    ggtitle("Inconsistent Record Entry by Service End Date") +
    ylab("Values") +
    scale_x_discrete("End Date", drop = "FALSE") +
    theme(panel.background=element_blank()))
