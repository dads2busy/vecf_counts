...
birth_year_mutli <-
  dss_customers_by_year_und_6[!is.na(year_of_birth), .(cnt = length(unique(year_of_birth)), dmg = "multi birth yr"), unique_id]

birth_month_mutli <-
  dss_customers_by_year_und_6[!is.na(month_of_birth), .(cnt = length(unique(month_of_birth)), dmg = "multi birth mo"), unique_id]

ethnicity_mutli <-
  dss_customers_by_year_und_6[!is.na(ethnicity_code) & ethnicity_code != 0, .(cnt = length(unique(ethnicity_code)), dmg = "multi ethnicity"), unique_id]
...
dmg_multi_wide <- dcast(dmg_multi, unique_id ~ dmg, value.var = "cnt", fun.aggregate = sum)
